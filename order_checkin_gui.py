...
def load_or_create_dataframe(file_path, expected_headers, default_dtype=str):
    if os.path.exists(file_path) and os.path.getsize(file_path) > 0:
        try:
            ...
            if missing_cols and os.path.getsize(file_path) > 0 :
                 QMessageBox.information(None,"File Schema Notice", f"File '{file_path}' was missing columns; they've been added if reading an existing file.")
        ...
        if not os.path.exists(file_path): QMessageBox.information(None, "File Notice", f"File '{file_path}' not found. Creating with headers.")
        elif os.path.getsize(file_path) == 0: QMessageBox.information(None, "File Notice", f"File '{file_path}' is empty. Initializing with headers.")
        try:
...
def append_to_csv(df_to_append, file_path, expected_headers):
    ...
    for col in expected_headers: df_ready[col] = df_to_append.get(col, pd.Series(dtype=str))
    df_ready = df_ready[expected_headers].astype(str) 
...
class OrderCheckInGUI(QMainWindow):
    def __init__(self):
        ...
        load_or_create_dataframe(STOCK_MOVEMENTS_FILE, STOCK_MOVEMENTS_HEADERS) 
        self.current_selected_order_line_df_index = None 
        self.init_ui(); self.refresh_pending_orders_table()

    # save_dataframe is NOW A METHOD
    ...
    def refresh_pending_orders_table(self):
        ...
        if 'Status' in self.order_history_df.columns:
            self.order_history_df['_status_lower'] = self.order_history_df['Status'].astype(str).str.lower() # Ensure string before .str
        ...
        for r in range(self.display_df.shape[0]):
            for c_idx, col_name in enumerate(cols):
                self.pending_table.setItem(r, c_idx, QTableWidgetItem(str(self.display_df.iloc[r].get(col_name, ''))))
    ...
    def on_order_line_selected(self):
        ...
        original_indices = self.order_history_df[
            ...
            (self.order_history_df['Status'].astype(str).str.lower().isin(["ordered", "partially received"])) # Ensure string for .str.lower()
        ].index.tolist()
        if not original_indices: QMessageBox.warning(self, "Error", "Could not find selected order line. Try refreshing."); self.clear_checkin_form(); return
    ...
    def process_receipt(self):
        ...
        try:
            ...
            self.save_dataframe(self.materials_df, MATERIALS_MASTER_FILE, MATERIALS_HEADERS) # Now uses self.save_dataframe
            self.save_dataframe(self.order_history_df, ORDER_HISTORY_FILE, ORDER_HISTORY_HEADERS) # Now uses self.save_dataframe
